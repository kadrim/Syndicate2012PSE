...
export class SessionController {
  ...
  async createSession(
    ...
    this.logger.debug(`[LOG] CREATE session request: ${JSON.stringify(request)}`);
  ...
  async getSession(
    ...
    this.logger.debug(`[LOG] GET session ${sessionId} for title ${titleId}`);
    ...
    if (!session) {
      this.logger.debug(`[LOG] GET session ${sessionId} NOT FOUND`);
      throw new NotFoundException(`Session ${sessionId} was not found.`);
    }

    const result = this.sessionMapper.mapToPresentationModel(session);
    this.logger.debug(`[LOG] GET session ${sessionId} response: ${JSON.stringify(result)}`);
    return result;
  }
  ...
  async getSessionDetails(
    ...
    this.logger.debug(`[LOG] GET session details ${sessionId} response: ${JSON.stringify(result)}`);
    return result;
  }
  ...
  async joinSession(
    ...
    this.logger.debug(`[LOG] JOIN session ${sessionId} xuids: ${JSON.stringify(request.xuids)}`);
  ...
  async sessionSearch(
    ...
    const result = sessions.map(this.sessionMapper.mapToPresentationModel);
    this.logger.debug(`[LOG] Session search results (${result.length}): ${JSON.stringify(result)}`);
    return result;
  }
